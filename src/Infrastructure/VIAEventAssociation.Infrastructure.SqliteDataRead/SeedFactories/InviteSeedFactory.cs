...
public static class InviteSeedFactory
{
    ...
    public static void Seed(VeadatabaseProductionContext context)
    {
        ...
        var tmpInvitations = JsonSerializer.Deserialize<List<TmpInvitation>>(
            InvitationsAsJson,
            new JsonSerializerOptions { PropertyNameCaseInsensitive = true }
        );
            
        // Group invitations by event for efficiency
        var invitationsByEvent = tmpInvitations
            .GroupBy(i => i.EventId)
            .ToDictionary(g => g.Key, g => g.ToList());
            
        // Process each event's invitations
        foreach (var eventId in invitationsByEvent.Keys)
        {
            AddInvitationsToEvent(
                context, 
                eventId, 
                invitationsByEvent[eventId]
            );
        }
    }
        
    private static void AddInvitationsToEvent(
        VeadatabaseProductionContext context,
        string eventId,
        List<TmpInvitation> tmpInvitations)
    {
        // Find invitations for this event
        var invitations = tmpInvitations
            .Where(i => i.EventId == eventId)
            .Select(invt => new Invite
            {
                Id = Guid.NewGuid().ToString(), // Generate a new unique ID
                EventId = invt.EventId,
                Status = invt.Status,
                AssignedGuestId = invt.GuestId
            });
            
        context.Invites.AddRange(invitations);
    }
        
    private record TmpInvitation(string EventId, string GuestId, string Status);
}
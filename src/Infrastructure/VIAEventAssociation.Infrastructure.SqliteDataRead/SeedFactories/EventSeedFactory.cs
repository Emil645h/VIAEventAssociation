...
public static class EventSeedFactory
{
    ...
    {
      List<TmpEvent> eventsTmp = JsonSerializer.Deserialize<List<TmpEvent>>(EventAsJson, 
      ...
      var events = eventsTmp.Select(e => new Event
      {
        ...
        StartTime = ConvertToIsoDateTime(e.Start),
        EndTime = ConvertToIsoDateTime(e.End)
      }).ToList();

      return events;
    }
    
    private static string ConvertToIsoDateTime(string dateTimeString)
    {
      if (string.IsNullOrEmpty(dateTimeString))
        return "";
            
      // Convert from "2024-03-01 15:00" to "2024-03-01T15:00:00"
    ...
    public record TmpEvent(string Id, string Description, int MaxGuests, string Title, string Status, string Visibility, string Start, string End);

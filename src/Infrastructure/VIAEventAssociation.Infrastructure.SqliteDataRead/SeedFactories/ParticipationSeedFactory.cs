...
namespace VIAEventAssociation.Infrastructure.SqliteDataRead.SeedFactories;
...
public static class ParticipationSeedFactory
{
    ...
    public static void Seed(VeadatabaseProductionContext context)
        {
            // Parse the participation data
            var participations = JsonSerializer.Deserialize<List<TmpParticipation>>(
                ParticipationsAsJson,
                new JsonSerializerOptions { PropertyNameCaseInsensitive = true }
            );
            
            // Group participations by event for efficiency
            var participationsByEvent = participations
                .GroupBy(p => p.EventId)
                .ToDictionary(g => g.Key, g => g.ToList());
            
            // Process each event's participations
            foreach (var eventId in participationsByEvent.Keys)
            {
        ...
        private static void AddParticipantsToEvent(
            VeadatabaseProductionContext context,
            string eventId,
            IEnumerable<TmpParticipation> participations)
        {
            // Find the event
            var veaEvent = context.Events.SingleOrDefault(e => e.Id == eventId);
            if (veaEvent == null) return;
            
            // Find the GuestList for this event
            var guestList = context.GuestLists.SingleOrDefault(gl => gl.EventId == eventId);
            if (guestList == null) return;
            
            foreach (var participation in participations)
            {
                // Find the guest
                var guest = context.Guests.SingleOrDefault(g => g.Id == participation.GuestId);
                if (guest == null) continue;
                
                // Create a GuestListEntry to connect the guest to the guest list
                ...
                if (!context.GuestListEntries.Any(e => 
                    e.GuestId == entry.GuestId && e.GuestListId == entry.GuestListId))
                {
                    context.GuestListEntries.Add(entry);
                }
            }
        }
        
        private record TmpParticipation(string EventId, string GuestId);
}